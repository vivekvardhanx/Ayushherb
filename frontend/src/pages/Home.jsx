...
import { firestore, incrementVisitCount, getVisitCount } from "../services/firebase";
...
const Home = () => {
    ...
    const getPlants = async () => {
      try {
        ...
        const plantData = querySnapshot.docs.map((doc) => ({
          _id: doc.id, // ✅ This is critical for rendering cards
          ...doc.data(),
        }));
        console.log("Fetched plants:", plantData); // Optional debug
        setPlants(plantData);
      } catch (err) {
  ...
  const openPopup = (plant) => {
    // Use the multimedia array directly if it exists, or create one from individual properties
    const multimedia = plant.multimedia || [
      plant.multimedia1,
      plant.multimedia2,
      plant.multimedia3,
      plant.multimedia4,
