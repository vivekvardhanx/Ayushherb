...
const CommunityForum = () => {
  ...
  const handleDislike = async (postId) => {
    ...
    const newDislikes = isDisliked ? (dislikes[postId] || 0) - 1 : (dislikes[postId] || 0) + 1;
      ...
      <div className="p-4 md:p-10 bg-gradient-to-b from-green-50 via-white to-green-100 min-h-screen">
        ...
        {user ? (
          ...
          >
            <textarea
              ...
              className="w-full h-36 p-4 text-lg border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-400 transition resize-none"
              required
            />
            <div className="flex justify-end mt-4">
              <button
                type="submit"
                className="bg-gradient-to-r from-green-500 to-green-700 text-white font-semibold py-2 px-6 rounded-full hover:scale-105 hover:shadow-md transition"
              >
                Post
              </button>
            </div>
        ...
        <div className="space-y-10">
          {posts.map((post) => (
            <div
              key={post.id}
              className="p-6 md:p-8 bg-white rounded-2xl shadow-lg border hover:shadow-xl transition-transform hover:-translate-y-1"
            >
                ...
                <div className="mt-6">
                  {post.replies?.length > 0 ? (
                    post.replies.map((reply, index) => (
                      <div
                        key={index}
                        className="ml-4 mb-4 p-4 bg-gray-50 rounded-xl border"
                      >
                  ...
                  {user && (
                    ...
                    >
                      <textarea
                        ...
                        className="w-full h-24 p-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-400 transition resize-none"
                        required
                      />
